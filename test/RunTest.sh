...
genRandPorts() {
  echo $$ | gawk '{ srand(systime()+$0*100) }END{ print int(rand()*(65524-1025))+1025; print int(rand()*(65524-1025))+1025; print int(rand()*(65524-1025))+1025 }'
}
...
run() {
  ...
  if [[ $added -ne 1 ]]; then
    ...
    then
      ...
      cp "$tmpdir/$cid.stdout" "$cid$ver.txt"
      echo "Failed:"
      ...
      echo "Output from Bob saved in folder as $cid$ver.txt"
    fi
    tput sgr0;
  fi
